...
def ml_page():
    ...
    def display_isolation_forest_results():
        st.subheader(
            "1 - Détection des comportements normaux et anormaux (Isolation Forest)"
        )
    ...
    def display_dbscan_results():
        ...
        fig = px.histogram(
            data, x="cluster", title="Distribution des clusters", color="cluster"
        )
        st.plotly_chart(fig)
        st.markdown("<br><br>", unsafe_allow_html=True)
    ...
    def cross_analysis():
        ...
        fig = px.imshow(
            ...
            title="Carte de chaleur des anomalies et clusters",
        )
        st.plotly_chart(fig)
        st.markdown("<br><br>", unsafe_allow_html=True)
    ...
    def cluster_exploration():
        ...
        st.write(
            "Cette analyse permet de comparer les résultats des deux méthodes et d'identifier les corrélations entre anomalies et clusters de comportement."
        )
        selected_cluster = st.selectbox(
            "Sélectionner un cluster", sorted(data["cluster"].unique())
        )
        ...
        selected_ipsrc = st.multiselect(
            "Filtrer par IP Source", all_ipsrc, default="Tout sélectionner"
        )
        ...
        selected_proto = st.multiselect(
            "Filtrer par Protocole", all_proto, default="Tout sélectionner"
        )
        ...
        selected_action = st.multiselect(
            "Filtrer par Action", all_action, default="Tout sélectionner"
        )
        ...
        heatmap_data = (
            filtered_data.groupby(["ipsrc", "ipdst"]).size().reset_index(name="count")
        )
